...
void setSlam(int argc,string cameraparmas_path,string direction_path,string map_path){
    ...
    if(argc==7){
        cout<<"111111111113222222222222221"<<endl;
        fstream file;
...
void startSlam(cv::Mat im,string record_path,int frameNumber){
    //保存图像--------------------------------------------------------------6
    cv::imwrite(record_path+"//"+to_string(frameNumber)+"_raw.jpg",im);
    cv::Mat posef2g= SLAM.process(im,cameraParams,frameNumber);
    if(posef2g.empty()){
        std::cerr<<"Frame111111111 "<<frameNumber<<" pose not found"<<std::endl;
...
{  
    ...
    for (int frameNumber = 0; keyPressed!=27;frameNumber++) {
        ...
        myHik->getData();
        cv::Mat im =  myHik->srcImage;        double t1 = ros::Time::now().toSec();
